...
| June 2026 | Phase 4 Step 5 | Customer Segment Labels Assigned Using RFM Scores | Cursor Notebook Cell 10 | Wrote assign_segment Python function with if-elif-else logic to map RFM score combinations to five named segments. Applied function to all 4,844 rows using df.apply(). Printed segment distribution as counts and percentages using value_counts(). | Named segments give business teams an immediately actionable label for each customer group — easier to act on than raw number scores. Each segment maps to a specific CRM retention or re-engagement strategy. | All 4,844 customers assigned to one of five segments — Champion, Loyal Customer, Promising, At Risk High Value, or Lost Customer. Segment distribution confirms retention crisis is business-wide. |
